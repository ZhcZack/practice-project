namespace g10 {
    ...
    class PuzzleGame {
        ...
        private setup() {
            ...
            const maxHeight = this.box.offsetHeight
            const margin = 10
        ...
        randomDisplay() {
            if (this.puzzles.length < 1) { return }
            const length = this.puzzles.length
            const number = length - 4 // 16 -- same as hard code
            const ps: number[] = []
            while (ps.length !== number) {
                let n = Math.ceil(Math.random() * (length - 1))
                if (ps.indexOf(n) >= 0) {
        ...
        private puzzlesMoving(item: HTMLElement, chosen: HTMLElement, timer: number) {
            // 两块puzzle都可以移动的情况
            ...
            clearInterval(timer)
            timer = setInterval(() => {
                let itemSpeedX = (startX - item.offsetLeft) / 10
                let itemSpeedY = (startY - item.offsetTop) / 10
                if (itemSpeedX > 0) {
                ...
                let chosenSpeedX = (endX - chosen.offsetLeft) / 10
                let chosenSpeedY = (endY - chosen.offsetTop) / 10
                if (chosenSpeedX > 0) {
        ...
        private mouseEvents() {
            ...
            function mouseupEvent(this: HTMLElement, e: MouseEvent) {
                ...
                // puzzle move
                const chosen = get(self.identifier + ' .imglist .chosen')
                const item = get(self.identifier + ' .imglist .item')
